function [effect,klower_star_mps,L_star_mps,GAMMA_star_mps,phi_star_mps,b_star_mps,klower_star_orig,L_star_orig,GAMMA_star_orig,...
    phi_star_orig,b_star_orig,pistar_mps,pistar_orig]=effect_of_mps_f(kmax_mps,kmin_mps,kmax_orig,kmin_orig,kval_mps,kval_orig_f,kval_orig_g,gdik_orig,fdik_orig,fdik_mps,...
    cost,betaa,p1,p2)
...
count=1;
for klower=kmin_mps:1:kmax_mps
    L(count)=(1-p1)*sum(fdik_mps(find(kval_mps==klower):end)); % probability that an informed buyer passes the info.
...
clear pi
 ...
 L=zeros(kmin_orig-kmin_orig+1,1);
 GAMMA=zeros(kmin_orig-kmin_orig+1,1);
 phi=zeros(kmin_orig-kmin_orig+1,1);
 b=zeros(kmin_orig-kmin_orig+1,1);
 pi=zeros(kmin_orig-kmin_orig+1,1);
