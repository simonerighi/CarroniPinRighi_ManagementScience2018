function [effect,klower_star_mps,L_star_mps,GAMMA_star_mps,phi_star_mps,b_star_mps,klower_star_orig,L_star_orig,GAMMA_star_orig,...
    phi_star_orig,b_star_orig,pistar_mps,pistar_orig]=effect_of_mps_g(...
    kmax_mps_g,kmin_mps_g,kmax_orig_f,kmin_orig_f,kval_mps_g,kval_orig_g,kval_orig_f,gdik_orig,fdik_orig,gdik_mps,cost,betaa,p1,p2,fdik_mps)
...
for klower=kmin_orig_f:1:kmax_orig_f
    ...
    GAMMA(count)=1-sum(gdik_mps.*((1-L(count)).^kval_mps_g));
    phi(count)=(1-p2)*sum(gdik_mps.*((1-(1-L(count)).^kval_mps_g)./(kval_mps_g.*L(count))));
    if klower > kmin_orig_f % this solves the problem of numerical precision
        if phi(count-1)>0.499999
            phi(count)=0.5;
        end
    end
...
klower_star_mps=IX+kmin_orig_f-1; % klowerbar_star
L_star_mps=L(IX);
